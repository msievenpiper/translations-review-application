...
export function registerProjectHandlers(): void {
  ...
  ipcMain.handle('projects:create', (_event, data: any) => {
    ...
    `).run(
      ...
      JSON.stringify(data.rubric_config ?? { accuracy: { weight: 40 }, fluency: { weight: 20 }, completeness: { weight: 30 }, tone: { weight: 10 } }),
      data.custom_rules ?? '',
    )
  ...
  ipcMain.handle('projects:update', (_event, id: string, data: any) => {
    const allowed = ['name', 'base_url', 'source_locale', 'target_locales', 'rubric_config', 'custom_rules']
