...
export function CommentsPanel({ issues, activeId, onIssueClick, onExport }: Props): JSX.Element {
          ...
          issues.map((issue) => (
            ...
            >
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span
                  className={`px-1.5 py-0.5 rounded text-[10px] border ${SEVERITY_STYLES[issue.severity] ?? SEVERITY_STYLES.medium}`}
                >
                  {issue.severity}
                </span>
              ...
              {issue.translated_text && (
                <p
                  className="text-gray-400 line-through mb-0.5 truncate"
                  title={issue.translated_text}
                >
                  {issue.translated_text}
                </p>
              )}
