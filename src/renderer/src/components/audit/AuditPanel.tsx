...
export function AuditPanel({ projectId, onResult, onProgress, onAuditedUrl }: Props): JSX.Element {
  ...
  async function handleAudit(): Promise<void> {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Audit failed. Check your API key and try again.')
      onProgress('')
    } finally {
      setRunning(false)
    }
  ...
  function handleFileBrowse(): void {
